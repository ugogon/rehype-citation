...
function options$1(options, log) {
...
function parseNativeTypes(input, dataType) {
  switch (dataType) {
    case 'Array':
      if (input.length === 0 || input.every(entry => type$1(entry) === '@csl/object')) {
...
function matchType(typeList = [], data) {
  for (const type of typeList) {
    if (types$2[type].predicate(data)) {
      return matchType(types$2[type].extensions, data) || type;
    }
  }
}

function type$1(input) {
...
function addTypeParser(format, {
  ...
  types$2[format] = object;

  if (extend) {
    const parentTypeParser = types$2[extend];
...
function hasTypeParser(type) {
  return Object.prototype.hasOwnProperty.call(types$2, type);
}
function removeTypeParser(type) {
  delete types$2[type];
  const typeLists = [...Object.keys(dataTypes).map(key => dataTypes[key]), ...Object.keys(types$2).map(type => types$2[type].extensions).filter(list => list.length > 0)];
...
function listTypeParser() {
  return Object.keys(types$2);
}
function treeTypeParser() {
  const attachNode = name => ({
    name,
    children: types$2[name].extensions.map(attachNode)
  });
...
class TypeParser {
  ...
  parseElementConstraint() {
    ...
    return !constraint ? [] : [input => input.every(entry => type$1(entry) === constraint)];
  }
...
function correctField(fieldName, value, bestGuessConversions) {
  const fieldType = [].concat(fieldTypes$1[fieldName]);

  switch (fieldTypes$1[fieldName]) {
    case NAME:
...
Object.assign(Cite$1.prototype, log, options$2, set$1, sort$1, get$3);
...
//@ts-nocheck
var output$1 = Object.assign({}, json, label$1);
...
var _fieldTypes = {
	...
	publisher: publisher,
	pubstate: pubstate,
	reprinttitle: reprinttitle,
	series: series,
	shortauthor: shortauthor,
	shorteditor: shorteditor,
	shorthand: shorthand,
	shorthandintro: shorthandintro,
...
var biblatex = new Translator([...aliases, ...nonSpec, {
  ...
  convert: {
    toTarget(type, subtype, typeKey) {
      ...
      return [types$1.source[type] || 'document', typeKey || subtype];
    },

    toSource(type, genre) {
      const sourceType = types$1.target[type] || 'misc';
